...
| R032 | Pane status detection: classify each active pane as idle, working, asking, done, or error based on terminal output analysis | active | Status engine correctly classifies Claude Code sessions and plain shell sessions; verified against live output samples | M005/S01 | — |
| R033 | Real-time status event bus: WebSocket channel pushes pane status transitions to all connected clients within 2 seconds | active | Status change in terminal output appears in API/WebSocket within 2s; multiple browser tabs receive update simultaneously | M005/S01 | — |
| R034 | Desktop activity indicators: workspace tabs show colored badge when background pane needs attention; pane headers show status badge | active | Badge appears on workspace tab when background pane transitions to "asking"; clears on workspace switch | M005/S02 | — |
| R035 | Browser notifications for pane state transitions (asking state) with user permission flow | active | Browser notification fires when pane enters "asking" state; permission prompt shown on first use; works in Chrome and Safari | M005/S02 | — |
| R036 | Phone minimap view: workspace shown as grid of tappable status cards matching actual layout proportions | active | Phone (< 480px) displays minimap with session name, host, type, and color-coded status per card | M005/S03 | — |
| R037 | Phone tap-to-zoom: tap minimap card to enter full-screen interactive terminal; back gesture returns to minimap | active | User taps card, terminal fills screen, keyboard input works, back returns to minimap | M005/S03 | — |
| R038 | Touch scroll-back in terminal panes on mobile Safari and Chrome | active | Scroll-back works via touch swipe without xterm mouse protocol capturing gestures | M005/S04 | — |
| R039 | Notification preferences stored in settings DB — per-workspace, per-status-type thresholds | active | User can enable/disable notifications per workspace; settings persist across sessions | M005/S02 | — |
